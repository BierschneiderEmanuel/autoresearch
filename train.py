...
# Model architecture
ASPECT_RATIO = 64       # model_dim =((depth * ASPECT_RATIO + HEAD_DIM - 1) // HEAD_DIM) * HEAD_DIM
HEAD_DIM = 64           # target head dimension for attention
WINDOW_PATTERN = "SSMM" # sliding window pattern: 4S+2M+1L only (reduce full-attn layers)
...
def build_model_config(vocab_size, depth=DEPTH):
    base_dim = depth * ASPECT_RATIO
    model_dim = ((base_dim + HEAD_DIM - 1) // HEAD_DIM) * HEAD_DIM
    num_heads = model_dim // HEAD_DIM
    # Find largest divisor of num_heads that is <= num_heads // 2
    n_kv = max(1, num_heads // 2)
    while n_kv > 1 and num_heads % n_kv != 0:
        n_kv -= 1
    return GPTConfig(
